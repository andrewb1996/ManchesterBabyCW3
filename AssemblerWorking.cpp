...
int loadCodeFile()
{
	...
	while(getline(inputFile, line))
	{
		...
		if(line.at(ignoreValue) != (';'))
		{
			...
			if(iterate < 256)
			{
				//the line is then stored in the code file and the negate value is increased
...
int convertCode() 
{
  	...
  	for (int i=0; i<256; i++) 
	{
    	...
    	codeFile[i] = current.substr(0,endOfLine);
	}	
...
string scanCode() 
{
	...
	for(int i=negate; i>=0; i--) 
	{
		...
		string placeholder = codeFile[i].substr(start,3);
		...
		if (isCorrect == false) 
		{
			...
			errorString = "ERROR: Line " + to_string(i) + ". there is a problem with opcode: " + placeholder;
			return errorString;
		}
			...
			if (placeholder == instructionSet[j][0]) 
			{
				...
				if (instructionSet[j][1] == "Yes")
				{
					...
					if (placeholder == "VAR") 
					{
						...
						foundAtVar = codeFile[i].find_first_not_of("-1234567890 \t",positionFound);
						
						//if the size_t variable is not the greatest possible value in the size of the code then
						if (foundAtVar != string::npos) 
						{
							//display an error message to the user stating which line and what the problem is.
							errorString = "ERROR: Line " + to_string(i) + ". There is a problem with the variable number; " + codeFile[i].substr(pos,32);
							return errorString;
						}
					}
				}			
	...
	errorString = compiler(variableNumber);
	return errorString;
	...
	for int i=negate; i>=0; i--) 
	{
		...
		else 
		{
			...
			for (int k=0; k<13; k++) 
			{
				...
				{
					//replace each line in th ecode file with the binary value of the instruction set
					...
					if (instructionSet[k][1] == "Yes") 
					{
						...
						positionFound = current.find_first_not_of(" \t",positionFound);
...
int main() 
{
	...
	cout << "\nThe output has been written to an external file called AssemblerComplete.txt, The baby simulator will now be opened...\n";
}